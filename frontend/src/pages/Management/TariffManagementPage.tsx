...
export function TariffManagementPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await apiClient.tariff.read({
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex justify-between items-center mt-2">
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Tariff</h1>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[1000px]">
            <thead className="bg-white/5 text-muted-foreground font-bold uppercase tracking-wider text-[10px]">
              <tr>
                ...
                <th className="px-6 py-4 border-b border-white/5">Tariff Name</th>
                <th className="px-6 py-4 border-b border-white/5">Price Per Unit</th>
                <th className="px-6 py-4 border-b border-white/5">VAT</th>
            ...
            <tbody className="divide-y divide-white/5 text-white/90">
              {loading ? (
                ...
                data.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-mono text-odyssey-blue text-xs">{row.tariffId || row.id || '-'}</td>
                    <td className="px-6 py-4 font-bold">{row.tariffName || row.name || '-'}</td>
                    <td className="px-6 py-4 font-bold text-odyssey-electric">₦{row.price || row.pricePerUnit || 0}/kWh</td>
                    <td className="px-6 py-4 text-muted-foreground">{row.tax || row.vat || 0}%</td>
                    <td className="px-6 py-4 text-muted-foreground max-w-[200px] truncate">{row.remark || '-'}</td>
              ...
              ) : (<tr><td colSpan={7} className="px-6 py-16 text-center text-muted-foreground italic text-sm">No tariffs found</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
