...
export function MeterTokenTaskPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await apiClient.get<any[]>('/operations/tasks/token', {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Meter Token Task</h1>
        <p className="text-sm text-muted-foreground mt-1">Audit trail of all remote token delivery operations</p>
      </div>
      ...
      <div className="glass p-5 rounded-2xl border border-odyssey-border/50 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shadow-lg">
        <DateRangePicker from={filters.from} to={filters.to} onFromChange={v => setFilters({ ...filters, from: v })} onToChange={v => setFilters({ ...filters, to: v })} />
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[1000px]">
            <thead className="bg-white/5 text-muted-foreground font-bold uppercase tracking-wider text-[10px]">
              <tr>
                ...
                <th className="px-4 py-4">Data Item</th>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                [1, 2, 3].map(i => <tr key={i} className="animate-pulse"><td colSpan={9} className="px-4 py-4"><div className="h-4 bg-white/5 rounded" /></td></tr>)
              ) : data.length > 0 ? (
                data.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-4 text-muted-foreground">{row.dataItem || 'Token Delivery'}</td>
                    <td className="px-4 py-4 font-mono text-[10px] break-all max-w-[150px]">{row.token || '-'}</td>
                    <td className="px-4 py-4">{row.stationId}</td>
                    <td className="px-4 py-4 text-center"><span className="px-2 py-0.5 rounded-full bg-green-500/10 text-green-500 text-[10px] border border-green-500/20">{row.status || 'FINISHED'}</span></td>
                    <td className="px-4 py-4 italic text-muted-foreground">{row.remark || '-'}</td>
                  </tr>
                ))
              ) : (<tr><td colSpan={9} className="px-4 py-12 text-center text-muted-foreground italic">No token tasks found</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
