...
export function MeterReadingTaskPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await apiClient.get<any[]>('/operations/tasks/reading', {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Meter Reading Task</h1>
        <p className="text-sm text-muted-foreground mt-1">Audit trail of all remote meter reading operations</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse min-w-[1000px]">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                data.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-4 text-muted-foreground">{row.dataItem || 'Meter Reading'}</td>
              ...
              ) : (<tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground italic">No reading tasks found</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
