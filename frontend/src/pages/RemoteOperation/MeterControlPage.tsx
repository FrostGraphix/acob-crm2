...
import { Search, Power, RefreshCw, X } from 'lucide-react';
...
export function MeterControlPage() {
  ...
  const handleSubmitTask = async () => {
    if (!selectedMeter || !reason || !authorizedBy) return;
    setSubmitting(true);
    try {
      await apiClient.post('/operations/control-task', {
      ...
      console.error('Failed to create control task', err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-display font-bold text-white tracking-tight">Meter Control</h1>
          <p className="text-sm text-muted-foreground mt-1">Switch meters on or off remotely</p>
        </div>
                  ...
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-orange-500/10 text-orange-400 hover:bg-orange-500 hover:text-white transition-all text-[11px] font-bold border border-orange-500/20 flex items-center gap-2 mx-auto"
                      >
                        <Power className="w-3 h-3" /> Add Task
                      </button>
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="glass rounded-2xl border border-odyssey-border/50 p-6 w-full max-w-md shadow-2xl space-y-5">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-display font-bold text-white">Meter Control Task</h2>
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Control Type</label>
                <div className="flex gap-3">
                  <button
                    onClick={() => setControlType('CONNECT')}
                    className={cn("flex-1 py-2.5 rounded-xl text-sm font-bold transition-all border",
                      controlType === 'CONNECT' ? "bg-green-500/20 text-green-400 border-green-500/40" : "bg-black/20 text-muted-foreground border-white/5 hover:bg-white/5")}
                  >
                    Switch On
                  </button>
                  <button
                    onClick={() => setControlType('DISCONNECT')}
                    className={cn("flex-1 py-2.5 rounded-xl text-sm font-bold transition-all border",
                      controlType === 'DISCONNECT' ? "bg-red-500/20 text-red-400 border-red-500/40" : "bg-black/20 text-muted-foreground border-white/5 hover:bg-white/5")}
                  >
                    Switch Off
                  </button>
                </div>
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Reason</label>
                <input type="text" value={reason} onChange={e => setReason(e.target.value)} placeholder="Enter reason..." className="w-full bg-black/20 border border-white/5 rounded-xl py-2.5 px-4 text-sm text-white focus:outline-none focus:ring-1 focus:ring-odyssey-electric/50" />
              </div>
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Authorized By</label>
                <input type="text" value={authorizedBy} onChange={e => setAuthorizedBy(e.target.value)} placeholder="Operator name..." className="w-full bg-black/20 border border-white/5 rounded-xl py-2.5 px-4 text-sm text-white focus:outline-none focus:ring-1 focus:ring-odyssey-electric/50" />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={handleSubmitTask} disabled={submitting || !reason || !authorizedBy} className="flex-1 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-bold hover:bg-orange-500/90 transition-all disabled:opacity-50">
