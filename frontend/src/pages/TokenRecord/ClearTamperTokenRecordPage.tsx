...
export function ClearTamperTokenRecordPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resp = await apiClient.get<any[]>('/tokens/records/clear-tamper', {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Clear Tamper Token Record</h1>
        <p className="text-sm text-muted-foreground mt-1">History of tamper-reset token operations</p>
      </div>
