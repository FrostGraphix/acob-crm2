...
export function ClearTamperTokenPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Clear Tamper Token</h1>
        <p className="text-sm text-muted-foreground mt-1">Generate tokens to reset tamper alarms</p>
      </div>
                ...
                data.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button className="px-3 py-1.5 rounded-lg bg-orange-500/10 text-orange-400 hover:bg-orange-500 hover:text-white transition-all text-[11px] font-bold border border-orange-500/20 flex items-center gap-2 mx-auto">
                        <ShieldAlert className="w-3 h-3" /> Reset
                      </button>
                    </td>
                  </tr>
                ))
