...
export function ClearCreditTokenPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="text-2xl font-display font-bold text-white tracking-tight">Clear Credit Token</h1>
        <p className="text-sm text-muted-foreground mt-1">Generate tokens to clear accumulated credit on meters</p>
      </div>
                ...
                data.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button className="px-3 py-1.5 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white transition-all text-[11px] font-bold border border-red-500/20 flex items-center gap-2 mx-auto">
                        <CreditCard className="w-3 h-3" /> Clear
                      </button>
                    </td>
                  </tr>
                ))
